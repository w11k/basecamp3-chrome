...
class TodoFromMessagePopup extends React.Component<TodoFromMessagePopupProps> {
    ...
    return (
      <div style={{ height: '100%', width: '100%', padding: '24px 36px', display: 'flex', flexDirection: 'column' }}>
                ...
                {group.options.map((o, j) =>
                  <div className="radio-buttons-container" style={{ overflow: 'hidden' }}>
                    ...
                    <label htmlFor={`r-${o.id}`}
                           style={{ fontSize: '0.9em', cursor: 'pointer', padding: '0.5em 1em', borderRadius: '1rem', float: 'left', textOverflow: 'ellipsis', width: '100%' }}>
                      {o.title}
                    </label>
                    <br/>
                  </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '12px' }}>
          <button type="submit" onClick={() => this.addTodo()} className="btn btn--small btn--add-icon btn--with-icon">
            create to-do
          </button>
        </div>
      </div>
